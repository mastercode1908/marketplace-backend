...
//    @Value("${sendgrid.from.name}")
    ...
    @Override
    public void sendCustomEmail(String toEmail, String subject, String emailContent) {
        try {
            ...
            // Don't throw - email failure shouldn't break app functionality
        }
    }
    ...
    public void sendNotificationEmailToUsers(String toEmail, Notification notification) {
        ...
        // SendGrid recommends plain text for better deliverability
        String content = """
                %s

                Xin chào,

                Bạn vừa nhận được một thông báo mới từ hệ thống Marketplace:

                Nội dung: %s

                Vui lòng truy cập trang web để xem chi tiết: %s

                Trân trọng,
                Marketplace System
                """.formatted(notification.getTitle(), notification.getMessage(), shopLink);
